    ...
    <div className='relative px-6 py-4 md:px-8 bg-white'>
        <div className='flex flex-col md:flex-row items-start gap-6'>
            ...
            <div className='w-full pt-16 md:pt-0 md:pl-36'>
                <div className='flex flex-col md:flex-row items-start justify-between'>
                    <div>
                        <div className='flex items-center gap-3'>
                            <h1 className='font-bold text-2xl text-gray-900'> {user.full_name} </h1>
                            <Verified className='w-5 h-5 text-blue-500'/> 
                        </div>
                        <p className='text-gray-600'>{user.username? `@${user.username}` : 'Add a username'}</p>
                    </div>

                    {/* {User can edit his or her profile if he is not on others profile} */}
                    {!profileId &&                    
                    <button onClick={() => setShowEdit(true)} className='flex items-center gap-2 cursor-pointer border border-gray-300 hover:bg-gray-50 px-4 py-2 rounded-lg font-medium transition-colors mt-4 md:mt-0'>
                ...
                <div className='flex itemcenter gap-6 border-t border-gray-200 pt-4'>
                    <div>
